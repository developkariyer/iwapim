...
SELECT
    ...
    CASE
        WHEN JSON_UNQUOTE(JSON_EXTRACT(line_item.value, '$.product.id')) REGEXP '^[0-9]+$'
        THEN CAST(JSON_UNQUOTE(JSON_EXTRACT(line_item.value, '$.product.id')) AS UNSIGNED)
        ELSE NULL
        END AS product_id,
    CASE
        WHEN JSON_UNQUOTE(JSON_EXTRACT(line_item.value, '$.variant.id')) REGEXP '^[0-9]+$'
        THEN CAST(JSON_UNQUOTE(JSON_EXTRACT(line_item.value, '$.variant.id')) AS UNSIGNED)
        ELSE NULL
        END AS variant_id,
    CAST(JSON_UNQUOTE(JSON_EXTRACT(line_item.value, '$.variant.price')) AS DECIMAL(10,2)) AS price,
    JSON_UNQUOTE(JSON_EXTRACT(json, '$.currencyCode')) AS currency,
    CAST(JSON_UNQUOTE(JSON_EXTRACT(line_item.value, '$.quantity')) AS UNSIGNED) AS quantity,
    JSON_UNQUOTE(JSON_EXTRACT(line_item.value, '$.name')) AS variant_title,
    CAST(JSON_UNQUOTE(JSON_EXTRACT(json, '$.currentTotalDiscountsSet.shopMoney.amount')) AS DECIMAL(10,2)) AS total_discount,
    ...
    CAST(JSON_UNQUOTE(JSON_EXTRACT(json, '$.currentTotalPriceSet.shopMoney.amount')) AS DECIMAL(10,2)) AS total_price,
    CAST(JSON_UNQUOTE(JSON_EXTRACT(json, '$.currentSubtotalPriceSet.shopMoney.amount')) AS DECIMAL(10,2)) AS subtotal_price,
...
WHERE
  ...
  AND JSON_UNQUOTE(JSON_EXTRACT(line_item.value, '$.variant.id')) != 'null'
    ...
    AND JSON_UNQUOTE(JSON_EXTRACT(line_item.value, '$.variant.id')) REGEXP '^[0-9]+$'
