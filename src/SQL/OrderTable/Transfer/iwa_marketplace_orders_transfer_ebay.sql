INSERT INTO iwa_marketplace_orders_line_items (
    ...
    fulfillments_status, fulfillments_status_control, customer_first_name, customer_last_name
)
SELECT
    :marketplaceType,
    :marketPlaceId,
    ...
    SUBSTRING_INDEX(JSON_UNQUOTE(JSON_EXTRACT(json, '$.buyer.buyerRegistrationAddress.fullName')), ' ', LENGTH(JSON_UNQUOTE(JSON_EXTRACT(json, '$.buyer.buyerRegistrationAddress.fullName'))) -
    LENGTH(REPLACE(JSON_UNQUOTE(JSON_EXTRACT(json, '$.buyer.buyerRegistrationAddress.fullName')), ' ', ''))) AS customer_first_name,
    SUBSTRING_INDEX(JSON_UNQUOTE(JSON_EXTRACT(json, '$.buyer.buyerRegistrationAddress.fullName')), ' ', -1) AS customer_last_name
FROM
