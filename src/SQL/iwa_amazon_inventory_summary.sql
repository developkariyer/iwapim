...
SELECT
    CASE
        WHEN (iwasku IS NULL OR iwasku = '') THEN asin
        ELSE iwasku
    END as iwasku,
    asin,
    CASE
...
RENAME TABLE iwa_amazon_inventory_summary_temp TO iwa_amazon_inventory_summary;
CREATE INDEX idx_iwasku ON iwa_amazon_inventory_summary (iwasku);
CREATE INDEX idx_asin ON iwa_amazon_inventory_summary (asin);
CREATE INDEX idx_warehouse ON iwa_amazon_inventory_summary (warehouse);
