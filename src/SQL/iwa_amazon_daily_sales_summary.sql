...
WITH RECURSIVE
    ...
    sales_data AS (SELECT JSON_EXTRACT(o.json, '$.OrderItems[*].ASIN')                                              AS asin_array,
                   ...
                   WHERE REGEXP_LIKE(o.order_id, '^[0-9]{3}-[0-9]{7}-[0-9]{7}$') -- Validate Amazon Order ID format
...
(
    purchase_date
) AS sale_date, -- Extract only the date
    SUM
(
    quantity_shipped
) AS total_quantity
...
RENAME
TABLE iwa_amazon_daily_sales_summary_temp TO iwa_amazon_daily_sales_summary;
