...
class BolConnector extends MarketplaceConnectorAbstract
{
    ...
    protected function downloadOfferReport($forceDownload = false)
    {
        ...
        if ($report === false || $forceDownload) {
            ...
            $response = $this->request('GET', static::$apiUrl['offerExportUrl'] . $entityId, ['headers' => ['Accept' => 'application/vnd.retailer.v10+csv']]);
            if ($response->getStatusCode() !== 200) {
    ...
    public function download($forceDownload = false)
    {
        ...
        $catalog = json_decode($variant->jsonRead('apiResponseJson'), true)['catalog'];
        echo "OFFER ID: " . $catalog . "\n";
